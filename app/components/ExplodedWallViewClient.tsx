...
// Extracted WallLayer component to properly use hooks
function WallLayer({
  component,
  layerPosition,
  explodedOffset,
  ...
  const shouldRenderStuds = component.hasStuds && studWallConfig && studWallConfig.type !== 'none';

  useFrame(() => {
    // Add delay for cascading effect during compression
    if (isCompressing && Date.now() - startTime < index * 100) {
      return;
    }
    ...
    if (Math.abs(diff) > 0.001) {
      ...
      if (isCompressing && index === totalLayers - 1) {
        onCompressionComplete();
      }
    }
  });
...
export default function ExplodedWallViewClient({ onNavigate }: ExplodedWallViewProps) {
